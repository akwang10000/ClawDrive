...
test("CodexCliProvider hard transport warnings do not wait behind the full default task budget", async () => {
  const provider = new CodexCliProvider(makeConfig({ tasksDefaultTimeoutMs: 300_000 }));
  const runtimeSignals: Array<{ code: string; severity: string; summary: string }> = [];
  const startedAt = Date.now();

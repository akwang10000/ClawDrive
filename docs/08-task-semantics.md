...
Provider evidence fields capture why a provider-backed run settled the way it did. Current evidence includes whether a provider turn started or completed, the finalization path, the final message source, the last agent-message preview when available, and the provider stdout event tail. These fields are diagnostic context, not a separate lifecycle.

Runtime signals summarize provider conditions observed during execution. Non-fatal signals may leave `executionHealth` as `warning` or `degraded`; fatal signals only produce task failure when no usable provider result or bounded local fallback can complete the request.

A task may finish cleanly when the selected provider returns a usable terminal result without material runtime degradation. A task may finish as `completed` with `executionHealth: "degraded"` when the selected provider failed, stalled, or returned unusable output but bounded local fallback produced a useful result. In that case `runtimeSignals` and `providerEvidence` explain the fallback, while `errorCode` remains null because the user received a usable terminal result.

For planning requests, the same degraded fallback can settle as `waiting_decision` when bounded local analysis can still produce safe choices. The task remains resumable from that decision state, and its provider evidence explains that the decision was generated by fallback rather than a clean provider completion.
...
- `waiting_decision` with `degraded` health because provider execution degraded but bounded local fallback produced a decision point
